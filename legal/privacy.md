...
サービスの提供に伴い、配信基盤はIPアドレスやリクエストに関する情報を通信処理のために扱います。本サービスがAWSに記録する運用ログの項目は、アクセス日時のみです。IPアドレス、リクエストのパス、入力内容を含むクエリ文字列、User-Agentは、このログに記録しません。

記録したアクセス日時は、本サービスの稼働状況の確認、障害調査の補助および安定運用のための負荷状況の把握に利用します。
...
- 入力内容・生成画像：アプリケーションによる永続保存は行わず、AWSの運用ログにも記録しません。
- 運用ログ（アクセス日時）：保存期間を90日間に設定し、期間経過後はAWSの保存期間設定に従って自動削除します。
